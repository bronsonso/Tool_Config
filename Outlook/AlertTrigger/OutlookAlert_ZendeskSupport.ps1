...
function Convert-TimestampToGMT8 {
    ...
    $convertedText = [regex]::Replace($InputText, 'Received:\s*([0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\+[0-9]{2}:[0-9]{2})', {
        ...
        $formattedTime = $gmtPlus8Time.ToString("yyyy-MM-dd HH:mm:ss")
...
foreach ($file in $files) {
    ...
    powershell.exe -ExecutionPolicy Bypass -File "C:\Users\bronson.so\Documents\EdCity\Repository\Tool\Config\Outlook\Common\PersistentPopup.ps1" `
